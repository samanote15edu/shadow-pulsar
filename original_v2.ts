...
serve(async (req) => {
  ...
  try {
    ...
    if (!profile) {
      ...
      // 1. Manejar C├│digo de Invitaci├│n
      if (!step) {
        ...
        await sendWhatsAppMessage(from, "≡ƒæï ┬íHola! Para unirte a Shadow Pulsar, por favor escribe tu c├│digo de invitaci├│n.");
        return new Response('OK', { status: 200 });
      }
      ...
      if (step === 'awaiting_store_name') {
        ...
        await sendWhatsAppButtons(from, `≡ƒÅ¬ *${text}*\n\n┬┐Qu├⌐ tipo de negocio es?`, [
          { id: 'inventory', title: 'Inventario ≡ƒôª' },
          { id: 'activity_logs', title: 'Bit├ícora ≡ƒô¥' }
        ]);
        return new Response('OK', { status: 200 });
      }
      ...
      if (step === 'awaiting_business_type') {
        ...
        if (store) {
          await supabase.from('profiles').insert({ whatsapp_number: from, role: 'owner', store_id: store.id, full_name: 'Due├▒o' });
          await supabase.from('registration_states').delete().eq('whatsapp_number', from);
          await sendWhatsAppMessage(from, `≡ƒÄë ┬íFelicidades! Tu negocio *${store.name}* ha sido registrado.\n\nYa puedes empezar escribiendo: 'Inventario'`);
        }
        return new Response('OK', { status: 200 });
      }
    ...
    // 4. L├ôGICA CONVERSACIONAL (NUEVA)
